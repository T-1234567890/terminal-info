...
pub fn run(options: ChatOptions) -> Result<(), String> {
    ...
    println!("Tip: /provider switch provider · /model switch model · /new new chat · /chats open saved chats\n");
    ...
    loop {
        let input = read_user_input(state.provider, &state.model)?;
        ...
        state
            .session
            .push_message(ChatRole::User, trimmed.to_string(), now);
        ...
        let provider_client = build_provider(
            &state.config,
            state.provider,
            state.model.clone(),
            state.system.clone(),
        )?;
        provider_client.send_message(request_messages, &mut |chunk| {
            renderer.push(chunk);
...
fn read_user_input(provider: ProviderKind, model: &str) -> Result<String, String> {
